# 软件需求规格说明书

> 1. 《软件需求规格说明》(SRS)描述对计算机软件配置项 CSCI的需求，及确保每个要求得以满足的所使用的方法。涉及该 CSCI外部接口的需求可在本 SRS 中给出：或在本 SRS引用的一个或多个《接口需求规格说明》(IRS)中给出。
> 2. 这个 SRS，可能还要用 IRS加以补充，是CSCI设计与合格性测试的基础。
> 3. 本课程及本实验不涉及、不强调、不关注的部分可省略，但不允许删除已有条目。
> 4. 最终文档请删除所有灰色字体和斜体等说明部分。
> 5. 高亮章节为重点关注章节。

[TOC]

## 1 范围

### 1.1 标识

本⽂档为"灵狐智验"在线实验平台软件需求规格说明书（SRS），版本号：1.0.0，发布⽇期：2025年4⽉20⽇。

### 1.2 系统概述

#### 适用系统：Windows、MacOS

#### 软件用途

"灵狐智验"是一款专为软件工程实践教学设计的在线实验平台，旨在解决魔法学院现有实验系统中存在的问题，如代码提交丢失、操作错误无法恢复等。该平台支持Web端访问，主要分为学生端与教师端，实现实验作业发布、提交、自动化评测与手动批改等核心功能。

#### 系统主要特性

- 用户权限管理
- 实验内容管理
- 自动化评测与即时反馈
- 进度保存与恢复功能
- 集成开发工具与资源共享
- 答疑论坛设置

#### 项目相关方

- 项目投资方与需方：魔法学院教务处  
- 用户：魔法学院师生  
- 开发方：灵狐软件开发团队  
- 支持机构：魔法学院信息中心

#### 相关文档：

- 项目2："灵狐智验"需求描述文档.pdf
- 软件开发计划书.md
- 软件概要设计说明书.md
- 软件详细设计说明书.md

### 1.3 文档概述

本文档是"灵狐智验"在线实验平台的软件需求规格说明书，主要用于：

1. 指导开发团队理解和实现系统功能
2. 作为验收测试的依据
3. 作为后期维护和升级的参考

本文档仅供项目相关方使用，未经授权不得外传。使用本文档时应当：
- 严格遵循文档中定义的接口规范
- 及时反馈文档中的问题和建议
- 定期同步最新版本

### 1.4 基线

本系统设计说明书基于以下基线：

1. 需求描述文档（2025年3月）
2. 软件开发计划书（2025年4月）

## 2 引用文件

本文档引用了以下文件：

1. 《项目2："灵狐智验"需求描述文档.pdf》
   - 发布日期：2025年3月20日
   - 来源：魔法学院教务处

2. 《软件开发计划书.md》
   - 版本：v1.0.0
   - 发布日期：2025年4月5日

3. 《软件工程 — 产品质量》(GB/T 16260-2006)

4. 《计算机软件文档编制规范》(GB/T 8567-2006)

## 3 需求

*本章应分以下几条描述 CSCI需求,也就是,构成 CSCI验收条件的 CSCI的特性。CSCI需求是为了满足分配给该 CSCI的系统需求所形成的软件需求。给每个需求指定项目唯一标识符以支持测试和可追踪性。并以一种可以定义客观测试的方式来陈述需求。如果每个需求有关的合格性方法和对系统(若适用,子系统)需求的可追踪性(见 5)在相应的章中没有提供,则在此进行注解。描述的详细程度遵循以下规则:应包含构成 CSCI验收条件的那些 CSCI特性,需方愿意推迟到设计时留给开发方说明的那些特性。如果在给定条中没有需求的话,本条应如实陈述。如果某个需求在多条中出现,可以只陈述一次而在其他条直接引用。*

### 3.1 所需的状态和方式

*如果需要 CSCI在多种状态和方式下运行,且不同状态和方式具有不同的需求的话,则要标识和定义每一状态和方式,状态和方式的例子包括:空闲、准备就绪、活动、事后分析、培训、降级、紧急情况和后备等。状态和方式的区别是任意的,可以仅用状态描述 CSCI,也可以仅用方式、方式中的状态,状态中的方式或其他有效方式描述。如果不需要多个状态和方式,不需人为加以区分,应如实陈述:如果需要多个状态或方式,还应使本规格说明中的每个需求或每组需求与这些状态和方式相关联,关联可在本条或本条引用的附录中用表格或其他的方法表示,也可在需求出现的地方加以注解。*

### 3.2 ==需求概述==

#### 3.2.1 目标

本系统的主要目标是：
1. 提供一个集成化的实验教学平台，解决传统实验教学中的痛点：
   - 实验过程难以追踪
   - 评分标准不统一
   - 师生互动效率低
   - 实验资源管理分散

2. 主要功能流程：
   - 教师创建实验 -> 发布实验 -> 学生执行实验 -> 自动评测 -> 成绩反馈
   - 支持实验全生命周期管理
   - 提供实时答疑和交流功能

3. 系统架构图：
```
+----------------+     +----------------+     +----------------+
|   用户界面层    |     |    业务逻辑层   |     |    数据存储层   |
+----------------+     +----------------+     +----------------+
| - Web前端界面   |     | - 实验管理模块  |     | - 用户数据     |
| - 移动端适配    |     | - 评测系统     |     | - 实验数据     |
| - 统一认证     |     | - 权限管理     |     | - 评测记录     |
+----------------+     +----------------+     +----------------+
```

#### 3.2.2 运行环境

硬件环境：
- 服务器：
  - CPU: 至少8核
  - 内存: 至少16GB
  - 存储: 至少1TB
- 客户端：支持主流浏览器的PC或移动设备

软件环境：
- 操作系统：WINDOEW
- 数据库：MySQL 8.0+
- Web服务器：Nginx
- 开发环境：Node.js, Java

#### 3.2.3 用户的特点

1. 教师用户：
   - 具备基本的计算机操作能力
   - 需要便捷的实验管理功能
   - 关注教学效果和学生表现

2. 学生用户：
   - 熟悉网络学习环境
   - 需要清晰的实验指导
   - 期望即时的实验反馈

#### 3.2.4 关键点

*说明本软件需求规格说明书中的关键点(例如:关键功能、关键算法和所涉及的关键技术等)。*

#### 3.2.5 约束条件

*列出进行本系统开发工作的约束条件。例如:经费限制、开发期限和所采用的方法与技术,以及政治、社会、文化、法律等。*

### 3.3 ==需求规格==

#### 3.3.1 软件系统总体功能/对象结构

```
实验管理平台
├── 用户管理
│   ├── 用户认证
│   ├── 角色管理
│   └── 个人中心
├── 实验管理
│   ├── 实验创建
│   ├── 实验发布
│   ├── 资源管理
│   └── 评测配置
├── 实验执行
│   ├── 在线编程
│   ├── 实验提交
│   └── 进度保存
├── 评测系统
│   ├── 自动评测
│   ├── 成绩管理
│   └── 评测记录
└── 交流系统
    ├── 答疑论坛
    ├── 即时通讯
    └── 公告管理
```
![uml.png](images\uml.png)
核心功能模块关键函数设计：

1. 用户管理模块
```typescript
class UserService {
    // 用户登录认证
    async authenticate(username: string, password: string): Promise<AuthResult> {
        // 1. 验证用户凭据
        // 2. 生成JWT令牌
        // 3. 返回用户信息和令牌
    }

    // 角色权限检查
    async checkPermission(userId: string, operation: string): Promise<boolean> {
        // 1. 获取用户角色
        // 2. 检查角色权限矩阵
        // 3. 返回权限判断结果
    }
}
```

2. 实验管理模块
```typescript
class ExperimentService {
    // 创建新实验
    async createExperiment(experimentData: ExperimentDTO): Promise<string> {
        // 1. 验证实验数据完整性
        // 2. 保存实验基本信息
        // 3. 处理实验资源文件
        // 4. 配置评测规则
        // 5. 返回实验ID
    }

    // 发布实验
    async publishExperiment(expId: string, config: PublishConfig): Promise<void> {
        // 1. 检查实验完整性
        // 2. 更新发布状态
        // 3. 创建学生实验实例
        // 4. 发送通知
    }
}
```

3. 实验执行模块
```typescript
class ExperimentExecutionService {
    // 保存实验进度
    async saveProgress(userId: string, expId: string, progress: ProgressData): Promise<void> {
        // 1. 验证提交权限
        // 2. 保存当前进度
        // 3. 更新时间戳
    }

    // 提交实验
    async submitExperiment(userId: string, expId: string, submission: SubmissionData): Promise<SubmitResult> {
        // 1. 检查提交时间
        // 2. 验证提交内容
        // 3. 触发自动评测
        // 4. 返回提交结果
    }
}
```

4. 评测系统模块
```typescript
class AutoGradingService {
    // 执行自动评测
    async grade(submissionId: string): Promise<GradingResult> {
        // 1. 获取评测规则
        // 2. 准备评测环境
        // 3. 运行测试用例
        // 4. 计算得分
        // 5. 生成评测报告
    }

    // 批量评测任务
    async batchGrade(submissionIds: string[]): Promise<BatchGradingResult> {
        // 1. 创建评测任务队列
        // 2. 并行执行评测
        // 3. 汇总评测结果
    }
}
```

5. 交流系统模块
```typescript
class CommunicationService {
    // 发布公告
    async publishAnnouncement(announcement: AnnouncementDTO): Promise<string> {
        // 1. 验证公告内容
        // 2. 保存公告
        // 3. 推送通知
    }

    // 创建讨论帖
    async createDiscussion(discussion: DiscussionDTO): Promise<string> {
        // 1. 检查权限
        // 2. 保存讨论内容
        // 3. 处理@提醒
    }
}
```

数据模型接口定义：

```typescript
interface ExperimentDTO {
    title: string;
    description: string;
    stages: Stage[];
    startTime: Date;
    endTime: Date;
    resources: Resource[];
    gradingRules: GradingRule[];
}

interface SubmissionData {
    expId: string;
    userId: string;
    content: {
        answers?: Answer[];
        code?: CodeSubmission[];
        files?: FileSubmission[];
    };
    timestamp: Date;
}

interface GradingResult {
    submissionId: string;
    score: number;
    feedback: string;
    testResults: TestCaseResult[];
    timestamp: Date;
}
```

关键业务流程示例：

1. 实验发布流程：
```typescript
async function publishExperimentWorkflow(expId: string, config: PublishConfig) {
    // 1. 权限检查
    await userService.checkPermission(currentUser, 'PUBLISH_EXPERIMENT');
    
    // 2. 获取实验信息
    const experiment = await experimentService.getExperiment(expId);
    
    // 3. 验证实验配置
    validateExperimentConfig(experiment);
    
    // 4. 创建学生实验实例
    await experimentService.createStudentInstances(expId, config.studentIds);
    
    // 5. 发送通知
    await notificationService.sendBatchNotifications({
        type: 'EXPERIMENT_PUBLISHED',
        targetUsers: config.studentIds,
        content: generateNotificationContent(experiment)
    });
}
```

2. 实验提交评测流程：
```typescript
async function submissionWorkflow(submission: SubmissionData) {
    // 1. 保存提交
    const submissionId = await submissionService.save(submission);
    
    // 2. 触发自动评测
    const gradingTask = await gradingService.createTask(submissionId);
    
    // 3. 执行评测
    const result = await gradingService.executeTask(gradingTask);
    
    // 4. 更新成绩
    await gradeService.updateGrade(submission.userId, submission.expId, result);
    
    // 5. 发送评测结果通知
    await notificationService.sendNotification({
        userId: submission.userId,
        type: 'GRADING_COMPLETE',
        content: generateGradingNotification(result)
    });
}
```

#### 3.3.2 软件子系统功能/对象结构

*对每个主要子系统中的基本功能模块/对象进行描述,包括结构图,流程图或对象图。*

#### 3.3.3 描述约定

*通常使用的约定描述(数学符号、度量单位等)。*

### 3.4 ==CSCI能力需求==

*本条应分条详细描述与 CSCI每一能力相关联的需求。"能力"被定义为一组相关的需求。可以用"功能"、"性能""主题""目标"或其他适合用来表示需求的词来替代"能力"。*

==包含核心需求即可==

#### 3.4.x （CSCI能力）

*本条应标识必需的每一个 CSC能力,并详细说明与该能力有关的需求。如果该能力可以更清晰地分解成若干子能力,则应分条对子能力进行说明。该需求应指出所需的 CSCI行为,包括适用的参数,如响应时间、吞吐时间、其他时限约束、序列、精度、容量(大小/多少)、优先级别、连续运行需求、和基于运行条件的允许偏差:(若适用)需求还应包括在异常条件,非许可条件或越界条件下所需的行为,错误处理需求和任何为保证在紧急时刻运行的连续性而引人到CSCI中的规定。在确定与CSCI所接收的输人和 CSCI所产生的输出有关的需求时,应考虑在本文 3.5.x给出要考虑的主题列表。*

*对于每一类功能或者对于每一个功能,需要具体描写其输人,处理和输出的需求。*

### 3.5 CSCI外部接口需求

#### 3.5.1 接口标识和接口图

灵狐智验平台的外部接口关系图：

```
+----------------+     +-----------------+     +------------------+
|   客户端浏览器  | <-> |   灵狐智验平台   | <-> |  外部集成系统    |
+----------------+     +-----------------+     +------------------+
        ^                      ^                       ^
        |                      |                       |
    用户界面API            认证服务API              集成服务API
```

#### 3.5.2 用户界面接口

1. Web前端接口规范
```typescript
interface UIInterface {
    // 基础请求格式
    baseRequest: {
        url: string;
        method: 'GET' | 'POST' | 'PUT' | 'DELETE';
        headers: {
            'Content-Type': 'application/json';
            'Authorization': string;
        };
    };

    // 响应格式
    baseResponse: {
        code: number;      // 200成功，其他表示错误
        message: string;   // 响应消息
        data: any;        // 响应数据
    };
}
```

2. 移动端适配接口
```typescript
interface MobileAdaptiveInterface {
    // 屏幕尺寸范围
    screenSizes: {
        xs: '320px-375px',
        sm: '376px-414px',
        md: '415px-768px',
        lg: '769px-1024px'
    };

    // 响应式布局断点
    breakpoints: {
        mobile: 414,
        tablet: 768,
        desktop: 1024
    };
}
```

#### 3.5.3 硬件接口

```typescript
interface HardwareInterface {
    // 存储接口要求
    storage: {
        minSpace: '100GB',
        type: 'SSD',
        iops: '≥10000'
    };

    // 网络接口要求
    network: {
        bandwidth: '≥100Mbps',
        latency: '≤50ms',
        protocol: ['TCP/IP', 'HTTP/HTTPS']
    };
}
```

#### 3.5.4 软件接口

1. 数据库接口
```typescript
interface DatabaseInterface {
    // MySQL连接配置
    mysql: {
        host: string;
        port: number;
        username: string;
        password: string;
        database: string;
        maxConnections: number;
    };

    // Redis缓存配置
    redis: {
        host: string;
        port: number;
        password: string;
        db: number;
    };
}
```

2. 评测系统接口
```typescript
interface JudgeSystemInterface {
    // Docker容器配置
    container: {
        image: string;
        cpuLimit: string;
        memoryLimit: string;
        timeout: number;
    };

    // 评测请求格式
    judgeRequest: {
        submissionId: string;
        language: string;
        code: string;
        testCases: TestCase[];
    };
}
```

#### 3.5.5 通信接口

1. RESTful API接口
```typescript
interface APIInterface {
    // API版本控制
    version: 'v1';
    
    // 认证头格式
    authHeader: {
        'Authorization': 'Bearer ${token}';
    };

    // 请求速率限制
    rateLimit: {
        maxRequests: 1000,
        timeWindow: '1m'
    };
}
```

2. WebSocket接口
```typescript
interface WebSocketInterface {
    // 实时通知
    notification: {
        event: string;
        data: any;
        timestamp: number;
    };

    // 在线编程
    codeSync: {
        roomId: string;
        userId: string;
        code: string;
        cursor: Position;
    };
}
```

### 3.6 CSCI内部接口需求

#### 3.6.1 模块间通信接口

1. 服务层接口
```typescript
interface ServiceInterface {
    // 实验管理服务
    ExperimentService: {
        createExperiment(data: ExperimentDTO): Promise<string>;
        publishExperiment(id: string): Promise<void>;
        getExperimentDetails(id: string): Promise<ExperimentDetails>;
    };

    // 评测服务
    GradingService: {
        submitForGrading(submission: SubmissionData): Promise<string>;
        getGradingResult(submissionId: string): Promise<GradingResult>;
        updateGradingStatus(submissionId: string, status: GradingStatus): Promise<void>;
    };
}
```

2. 事件总线接口
```typescript
interface EventBusInterface {
    // 事件定义
    events: {
        'experiment.created': ExperimentCreatedEvent;
        'submission.graded': SubmissionGradedEvent;
        'notification.sent': NotificationSentEvent;
    };

    // 事件处理器
    handlers: {
        onExperimentCreated(event: ExperimentCreatedEvent): Promise<void>;
        onSubmissionGraded(event: SubmissionGradedEvent): Promise<void>;
        onNotificationSent(event: NotificationSentEvent): Promise<void>;
    };
}
```

#### 3.6.2 数据访问接口

1. 数据库访问接口
```typescript
interface RepositoryInterface {
    // 基础CRUD操作
    BaseRepository<T> {
        create(entity: T): Promise<string>;
        findById(id: string): Promise<T>;
        update(id: string, data: Partial<T>): Promise<void>;
        delete(id: string): Promise<void>;
    };

    // 实验仓库
    ExperimentRepository extends BaseRepository<Experiment> {
        findByTeacher(teacherId: string): Promise<Experiment[]>;
        findActiveExperiments(): Promise<Experiment[]>;
    };
}
```

2. 缓存访问接口
```typescript
interface CacheInterface {
    // 缓存操作
    cache: {
        set(key: string, value: any, ttl?: number): Promise<void>;
        get(key: string): Promise<any>;
        delete(key: string): Promise<void>;
    };

    // 缓存策略
    strategy: {
        type: 'LRU' | 'LFU';
        maxSize: number;
        defaultTTL: number;
    };
}
```

#### 3.6.3 安全接口

```typescript
interface SecurityInterface {
    // 认证接口
    auth: {
        validateToken(token: string): Promise<UserContext>;
        refreshToken(refreshToken: string): Promise<AuthTokens>;
    };

    // 权限检查接口
    permission: {
        checkAccess(userId: string, resource: string, action: string): Promise<boolean>;
        getRolePermissions(roleId: string): Promise<Permission[]>;
    };
}
```

#### 3.6.4 日志接口

```typescript
interface LoggingInterface {
    // 日志级别
    levels: {
        DEBUG: 0,
        INFO: 1,
        WARN: 2,
        ERROR: 3
    };

    // 日志记录
    logger: {
        log(level: LogLevel, message: string, meta?: any): void;
        error(error: Error, context?: any): void;
        audit(action: string, userId: string, details: any): void;
    };
}
```

### 3.7 ==CSCI内部数据需求==

本系统的核心数据涵盖用户、实验、提交、评测、交流等多个维度，需满足高并发访问、一致性保障、安全合规和可扩展性等要求。结合系统架构、功能模块和接口设计，具体需求如下：

#### 3.7.1 核心数据结构设计

##### 1. 用户数据

| 字段名     | 数据类型     | 描述                       | 约束        |
|-----------|-------------|----------------------------|------------|
| userId    | VARCHAR(36) | 用户唯一标识                | 主键        |
| role      | ENUM        | 用户角色(教师/学生)          | 非空        |
| name      | VARCHAR(50) | 用户姓名                   | 非空        |
| email     | VARCHAR(100)| 电子邮箱                   | 唯一, 非空   |
| class     | VARCHAR(50) | 班级信息                   | 学生必填     |
| createTime| DATETIME    | 创建时间                   | 非空        |
| lastLogin | DATETIME    | 最后登录时间                | 可空        |

```sql
CREATE TABLE users (
    userId VARCHAR(36) PRIMARY KEY,
    role ENUM('TEACHER', 'STUDENT') NOT NULL,
    name VARCHAR(50) NOT NULL,
    email VARCHAR(100) NOT NULL UNIQUE,
    class VARCHAR(50),
    createTime DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    lastLogin DATETIME,
    CHECK (role = 'STUDENT' AND class IS NOT NULL OR role = 'TEACHER')
);
```

##### 2. 实验数据

| 字段名       | 数据类型      | 描述                    | 约束        |
|-------------|--------------|------------------------|------------|
| expId       | VARCHAR(36)  | 实验唯一标识             | 主键        |
| title       | VARCHAR(100) | 实验标题                 | 非空        |
| description | TEXT         | 实验描述                 | 非空        |
| stages      | JSON         | 实验阶段信息(数组)        | 非空        |
| startTime   | DATETIME     | 开始时间                 | 非空        |
| endTime     | DATETIME     | 结束时间                 | 非空        |
| creatorId   | VARCHAR(36)  | 创建者ID                | 外键        |
| status      | ENUM         | 实验状态                 | 非空        |

```sql
CREATE TABLE experiments (
    expId VARCHAR(36) PRIMARY KEY,
    title VARCHAR(100) NOT NULL,
    description TEXT NOT NULL,
    stages JSON NOT NULL,
    startTime DATETIME NOT NULL,
    endTime DATETIME NOT NULL,
    creatorId VARCHAR(36) NOT NULL,
    status ENUM('DRAFT', 'PUBLISHED', 'CLOSED') NOT NULL DEFAULT 'DRAFT',
    FOREIGN KEY (creatorId) REFERENCES users(userId),
    CHECK (endTime > startTime)
);
```


##### 3. 提交记录

| 字段名     | 数据类型      | 描述                    | 约束        |
|-----------|--------------|------------------------|------------|
| submitId  | VARCHAR(36)  | 提交唯一标识             | 主键        |
| userId    | VARCHAR(36)  | 用户ID                  | 外键,非空   |
| expId     | VARCHAR(36)  | 实验ID                  | 外键,非空   |
| stageId   | VARCHAR(36)  | 实验阶段ID               | 非空        |
| content   | JSON         | 提交内容                 | 非空        |
| score     | DECIMAL(5,2) | 得分                    | 可空        |
| feedback  | TEXT         | 评测反馈                 | 可空        |
| timestamp | DATETIME     | 提交时间戳               | 非空        |
| status    | ENUM         | 提交状态                 | 非空        |

```sql
CREATE TABLE submissions (
    submitId VARCHAR(36) PRIMARY KEY,
    userId VARCHAR(36) NOT NULL,
    expId VARCHAR(36) NOT NULL,
    stageId VARCHAR(36) NOT NULL,
    content JSON NOT NULL,
    score DECIMAL(5,2),
    feedback TEXT,
    timestamp DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    status ENUM('PENDING', 'GRADING', 'COMPLETED', 'ERROR') NOT NULL DEFAULT 'PENDING',
    FOREIGN KEY (userId) REFERENCES users(userId),
    FOREIGN KEY (expId) REFERENCES experiments(expId)
);
```

##### 4. 评测与成绩数据

| 字段名       | 数据类型      | 描述                    | 约束        |
|-------------|--------------|------------------------|------------|
| gradingId   | VARCHAR(36)  | 评测唯一标识             | 主键        |
| submitId    | VARCHAR(36)  | 提交ID                  | 外键,非空   |
| testResults | JSON         | 测试用例结果             | 非空        |
| totalScore  | DECIMAL(5,2) | 总分                    | 非空        |
| startTime   | DATETIME     | 评测开始时间             | 非空        |
| endTime     | DATETIME     | 评测结束时间             | 可空        |
| errorMsg    | TEXT         | 错误信息                 | 可空        |

```sql
CREATE TABLE gradings (
    gradingId VARCHAR(36) PRIMARY KEY,
    submitId VARCHAR(36) NOT NULL,
    testResults JSON NOT NULL,
    totalScore DECIMAL(5,2) NOT NULL DEFAULT 0,
    startTime DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    endTime DATETIME,
    errorMsg TEXT,
    FOREIGN KEY (submitId) REFERENCES submissions(submitId)
);
```

##### 5. 数据关系与一致性

本系统的数据关系图如下：

```
+----------+      +---------------+      +---------------+
|  users   | 1---*| experiments   |*----*| submissions   |
+----------+      +---------------+      +---------------+
      |                  |                       |
      |                  |                       |
      *                  *                       1
+----------+      +---------------+      +---------------+
| profiles |      | exp_resources |      | gradings      |
+----------+      +---------------+      +---------------+
```

#### 3.7.2 数据存储与访问策略

1. **分层存储策略**
   - 热点数据（如活跃实验、当前提交）存储在高速缓存
   - 冷数据（如历史实验、旧提交记录）迁移至低成本存储

2. **数据访问控制**
   - 基于角色的访问控制(RBAC)系统
   - 实验数据访问权限矩阵：
     * 教师：可创建、修改、查看实验与所有提交
     * 学生：可查看已发布实验，只能查看/提交自己的提交

3. **备份与恢复策略**
   - 实时数据库事务日志
   - 每日增量备份，每周全量备份
   - 最长恢复时间目标(RTO)：2小时

#### 3.7.3 数据安全与合规

1. **数据加密**
   - 敏感数据（如用户凭证）使用单向哈希加盐存储
   - 传输过程使用TLS 1.3加密

2. **隐私保护**
   - 符合相关教育数据保护法规
   - 用户数据匿名化处理策略

3. **审计日志**
   - 记录所有关键数据操作
   - 日志保留期至少6个月

本系统所有内部数据的设计与管理，需满足高并发、强一致性、安全合规等要求，为平台的稳定运行和后续扩展提供坚实基础。

### 3.8 适应性需求

*(若有)本条应指明要求 CSCI提供的、依赖于安装的数据有关的需求(如:依赖现场的经纬度)和要求 CSCI使用的,根据运行需要进行变化的运行参数(如:表示与运行有关的目标常量或数据记录的参数)。*

### 3.9 保密性需求

*(若有)本条应描述有关防止对人员、财产、环境产生潜在的危险或把此类危险减少到最低的CSCI需求,包括:为防止意外动作(如意外地发出"自动导航关闭"命令)和无效动作(发出一个想要的"自动导航关闭"命令时失败)CSCI必须提供的安全措施。*

### 3.10 保密性和私密性需求

*(若有)本条应指明保密性和私密性的 CSCI需求,包括:CSCI运行的保密性/私密性环境、提供的保密性或私密性的类型和程度、CSCI必须经受的保密性/私密性的风险、减少此类危险所需的安全措施、CSCI必须遵循的保密性/私密性政策、CSCI必须提供的保密性/私密性审核,保密性/私密性必须遵循的确证/认可准则。*

### 3.11 ==CSCI环境需求==

*(若有)本条应指明有关 CSCI必须运行的环境的需求。例如,包括用于 CSCI运行的计算机硬件和操作系统(其他有关计算机资源方面的需求在下条中描述)。*

1. 开发环境
   - 前端：Vue.js 3.0
   - 后端：Spring Boot
   - 数据库：MySQL + Redis
   - 评测系统：Docker容器化

2. 运行环境
   - Web服务器：Nginx 1.18+
   - 应用服务器：JDK 11+
   - 数据库服务器：MySQL 8.0+
   - 缓存服务器：Redis 6.0+

### 3.12 计算机资源需求

#### 3.12.1 计算机硬件需求

*本条应描述 CSCI使用的计算机硬件需求,(若适用)包括,各类设备的数量、处理器,存储器、输人/输出设备、辅助存储器、通信/网络设备和其他所需的设备的类型,大小,容量及其他所要求的特征。*

#### 3.12.2 计算机硬件资源利用需求

*本条应描述 CSCI计算机硬件资源利用方面的需求,如:最大许可使用的处理器能力、存储器容量、输入/输出设备能力、辅助存储器容量、通信/网络设备能力。描述(如每个计算机硬件资源能力的百分比)还包括测量资源利用的条件。*

#### 3.12.3 计算机软件需求

*本条应描述 CSCI必须使用或引人 CSCI的计算机软件的需求,例如包括:操作系统、数据库管理系统、通信/网络软件、实用软件、输入和设备模拟器、测试软件、生产用软件。必须提供每个软件项的正确名称、版本、文档引用。*

#### 3.12.4 计算机通信需求

*本条应描述 CSCI必须使用的计算机通信方面的需求,例如包括:连接的地理位置、配置和网络拓扑结构、传输技术、数据传输速率、网关、要求的系统使用时间、传送/接收数据的类型和容量、传送/接收/响应的时间限制、数据的峰值、诊断功能。*

### 3.13 ==软件质量因素==

==次要关注点==

*(若有)本条应描述合同中标识的或从更高层次规格说明派生出来的对CSCI的软件质量方面的需求,例如包括有关 CSCI的功能性(实现全部所需功能的能力)、可靠性(产生正确、一致结果的能力)、可维护性(易于更正的能力)、可用性(需要时进行访问和操作的能力)、灵活性(易于适应需求变化的能力)、可移植性(易于修改以适应新环境的能力)、可重用性(可被多个应用使用的能力)、可测试性(易于充分测试的能力),易用性(易于学习和使用的能力)以及其他属性的定量需求。*

### 3.14 设计和实现的约束

*(若有)本条应描述约束CSCI设计和实现的那些需求。这些需求可引用适当的标准和规范例如需求包括：*

- *特殊 CSCI体系结构的使用或体系结构方面的需求,例如:需要的数据库和其他软件配置项；标准部件、现有的部件的使用；需方提供的资源(设备、信息、软件)的使用；*
- *特殊设计或实现标准的使用;特殊数据标准的使用;特殊编程语言的使用；*
- *为支持在技术、风险或任务等方面预期的增长和变更区域,必须提供的灵活性和可扩展性。*

### 3.15 数据

*说明本系统的输入,、输出数据及数据管理能力方面的要求(处理量、数据量)。*

### 3.16 操作

*说明本系统在常规操作、特殊操作以及初始化操作、恢复操作等方面的要求。*

### 3.17 故障处理

*说明本系统在发生可能的软硬件故障时,对故障处理的要求。包括：*

- *说明属于软件系统的问题；*
- *给出发生错误时的错误信息；*
- *说明发生错误时可能采取的补救措施。*

### 3.18 算法说明

*用于实施系统计算功能的公式和算法的描述。包括:*

- *每个主要算法的概况；*
- *用于每个主要算法的详细公式。*

### 3.19 有关人员需求

*(若有)本条应描述与使用或支持 CSCI的人员有关的需求,包括人员数量、技能等级、责任期、培训需求,其他的信息。如:同时存在的用户数量的需求,内在帮助和培训能力的需求,(若有)还应包括强加于 CSCI的人力行为工程需求,这些需求包括对人员在能力与局限性方面的考虑:在正常和极端条件下可预测的人为错误,人为错误造成严重影响的特定区域,例如包括错误消息的颜色和持续时间、关键指示器或关键的物理位置以及听觉信号的使用的需求。*

### 3.20 有关培训需求

*(若有)本条应描述有关培训方面的CSCI需求。包括:在 CSCI中包含的培训软件。*

### 3.21 有关后勤需求

*(若有)本条应描述有关后勤方面的 CSCI需求,包括:系统维护、软件支持、系统运输方式、供应系统的需求、对现有设施的影响、对现有设备的影响。*

### 3.22 其他需求

*(若有)本条应描述在以上各条中没有涉及到的其他CSCI需求。*

### 3.23 包装需求

*(若有)本条应描述需交付的 CSCI在包装、加标签和处理方面的需求(如用确定方式标记和包装8磁道磁带的交付)。(若适用)可引用适当的规范和标准。*

### 3.24 需求的优先次序和关键程度

*(若适用)本条应给出本规格说明中需求的、表明其相对重要程度的优先顺序、关键程度或赋予的权值,如:标识出那些认为对安全性、保密性或私密性起关键作用的需求,以便进行特殊的处理如果所有需求具有相同的权值,本条应如实陈述。*

## 4 合格性规定

*本章定义一组合格性方法,对于第3章中每个需求,指定所使用的方法,以确保需求得到满足可以用表格形式表示该信息,也可以在第3章的每个需求中注明要使用的方法。合格性方法包括：*

- *演示:运行依赖于可见的功能操作的 CSCI或部分 CSCI,不需要使用仪器,专用测试设备或进行事后分析；*
- *测试.使用仪器或其他专用测试设备运行 CSCI或部分 CSCI,以便采集数据供事后分析使用；*
- *分析:对从其他合格性方法中获得的积累数据进行处理,例如测试结果的归约、解释或推断；*
- *审查:对 CSCI代码、文档等进行可视化检查；*
- *特殊的合格性方法。任何应用到CSCI的特殊合格性方法,如:专用工具、技术、过程、设施、验收限制。*

## 5 需求可追踪性

*本章应包括：*

- *从本规格说明中每个 CSCI的需求到其所涉及的系统(或子系统)需求的可追踪性。(该可追踪性也可以通过对第3章中的每个需求进行注释的方法加以描述)*

  *注:每一层次的系统细化可能导致对更高层次的需求不能直接进行追踪。例如:建立多个 CSCI的系统体系结构设计可能会产生有关 CSCI之间接口的需求,而这些接口需求在系统需求中并没有被覆盖,这样的需求可以被追踪到诸如"系统实现"这样的一般需求,或被追踪到导致它们产生的系统设计决策上。*

- *从分配到被本规格说明中的 CSCI的每个系统(或子系统)需求到涉及它的 CSCI需求的可0.追踪性。分配到 CSC的所有系统(或子系统)需求应加以说明。追踪到IRS中所包含的CSCI需求可引用 IRS。*

## 6 ==尚未解决的问题==

1. 性能优化
   - 大规模并发提交时的评测系统性能
   - 文件存储系统的扩展性

2. 安全性
   - 代码执行环境的隔离方案
   - 防作弊机制的具体实现

3. 集成问题
   - 与学校现有教务系统的对接方案
   - 第三方工具（如Git）的集成方式

## 7 注解

*本章应包含有助于理解本文档的一般信息(例如背景信息、词汇表、原理》。本章应包含为理解本文档需要的术语和定义,所有缩略语和它们在文档中的含义的字母序列表。*

## 附录

*附录可用来提供那些为便于文档维护而单独出版的信息(例如图表、分类数据)。为便于处理附录可单独装订成册。附录应按字母顺序(A,B等)编排。*
